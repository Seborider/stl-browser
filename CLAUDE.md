...
Phase 7 (macOS polish: Tailwind v4 light/dark surfaces driven by `prefers-color-scheme` with `theme: null` in tauri.conf and opaque window chrome — vibrancy/NSVisualEffectView was attempted via `window-vibrancy` but rolled back because WKWebView kept painting an opaque dark canvas in dark mode; "Reveal in Finder" IPC command shells out through `tauri-plugin-shell` to `open -R` with an absolute-path capability scope; `font-sans` left on Tailwind's default stack so macOS resolves SF Pro; Space toggles the detail viewer from the grid and closes it when open, wired through `useKeyboardNav`) complete. Next: Phase 8 — ship prep (signing, notarization, bundler config, icons). See `PLAN.md` §2 for the full phased roadmap.
