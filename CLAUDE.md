...
Phase 7.6 (detail-viewer appearance controls: model color, light color, light azimuth — live in the detail viewer, persisted globally in the existing `settings` k/v table without a schema bump; `Preferences` extended with `modelColor` / `lightColor` / `lightAzimuthDeg`; three IPC setters mirror `set_language`; React updates Zustand synchronously and debounces SQLite writes 250 ms; three.js mutations are imperative through refs into the single uniform `MeshStandardMaterial` plus the `directionalLight` ref, so drag stays at 60 fps without r3f re-renders; elevation is fixed at 45°; UI uses `@radix-ui/react-popover` directly — full shadcn machinery skipped to avoid the components.json + path-alias overhead for three controls) complete. Phase 7.5 (i18n + native-menu Settings) preceded it. Next: Phase 8 — ship prep (signing, notarization, bundler config, icons). See `PLAN.md` §2 for the full phased roadmap.
