...
Phase 8 (ship prep) **partial** — bundler config, Info.plist, and `.icns` done; Developer ID signing and notarization deferred to a follow-up. `src-tauri/tauri.conf.json` carries `bundle.category: "Productivity"`, `copyright`, `shortDescription`/`longDescription`, and a `bundle.macOS` block with `minimumSystemVersion: 12.0` plus an explicit `dmg` window layout (660×400, app at 180,170, Applications folder at 480,170 — drag-to-Applications hint). Folder-access usage strings (`NSDesktopFolderUsageDescription`, `NSDocumentsFolderUsageDescription`, `NSDownloadsFolderUsageDescription`, `NSRemovableVolumesUsageDescription`) plus `NSHumanReadableCopyright` and `NSHighResolutionCapable` live in `src-tauri/Info.plist`, which Tauri auto-merges by sibling-file convention (no path field needed in `tauri.conf.json`). The boilerplate Tauri logo is replaced everywhere: `src/assets/stl.png` (1024×1024) is the source, `src-tauri/icons/icon.icns` is generated via `sips` + `iconutil` from a 10-size iconset, and `32x32.png` / `128x128.png` / `128x128@2x.png` are re-`sips`'d from the same source. `pnpm tauri build --target aarch64-apple-darwin` produces `STL Browser.app` and `STL Browser_0.1.0_aarch64.dmg` under `src-tauri/target/aarch64-apple-darwin/release/bundle/{macos,dmg}/`. Phase 7.7 preceded it. Next: finish Phase 8 — Developer ID signing, `entitlements.plist` with `com.apple.security.cs.allow-jit` + `allow-unsigned-executable-memory`, `notarytool` pipeline, and a stapled, notarized DMG. See `PLAN.md` §2 and §8 for the full phased roadmap.
