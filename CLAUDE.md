...
Phase 7.5 (i18n: German + English with the macOS native menu bar as the settings surface — top-level `Settings` menu contains nested `Theme` and `Language` submenus; menu strings come from a static `match` map in `src-tauri/src/i18n.rs`, React strings from `react-i18next` flat catalogs at `src/i18n/locales/{en,de}.json` grouped by area; `sys-locale` resolves the OS locale on first launch and persists the result in the existing SQLite `settings` table alongside `theme_mode`; the language preference flows through `get_preferences` + `set_language` IPC and the `language:changed` event; `useLanguage` hook mirrors `useTheme` and `main.tsx` bootstraps i18next before React mounts to avoid an en→de flash; menu rebuild via `app.set_menu(menu::build(...))` on language change so labels relabel immediately) complete. Next: Phase 8 — ship prep (signing, notarization, bundler config, icons). See `PLAN.md` §2 for the full phased roadmap.
