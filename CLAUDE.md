...
Phase 6 (detail viewer: double-click a tile → R3F `<Canvas>` overlay with drei `OrbitControls`, ambient + directional lights, 220×220 mm print-bed grid on XY, camera framed to bounding sphere with Z-up; `MeshLoader` parses STL / OBJ / 3MF on the main thread — no DOMParser polyfill needed there, unlike the worker; wireframe + flat/smooth toggles via buttons and `W` / `S` shortcuts; close via button or `Esc`, grid stays mounted behind `position: fixed` so scroll position is preserved) complete. Next: Phase 7 — macOS polish. See `PLAN.md` §2 for the full phased roadmap.
